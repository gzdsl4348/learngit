...
static hwlock_t memory_lock = 0;
#define MEN_LOCK_INIT()     memory_lock = hwlock_alloc()
#define MEM_LOCK()          hwlock_acquire(memory_lock)
#define MEM_UNLOCK()        hwlock_release(memory_lock)
...
{  
	...
	u16 cmemb=0;//连续空内存块数
    u32 i;  
    if(!mallco_dev.memrdy)mem_init();	        //mallco_dev.init未初始化,先执行初始化
