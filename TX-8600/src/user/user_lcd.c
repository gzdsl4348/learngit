...
// 没有任务
#if ENGLISH_VERSION
#define C_NONE_TASK_LEN 14
uint8_t none_task_char[14] = {0x00,0x4E,0x00,0x6F,0x00,0x20,0x00,0x74,0x00,0x61,0x00,0x73,0x00,0x6B};
#else
#define C_NONE_TASK_LEN 10
uint8_t none_task_char[10] = {0x6C,0xA1,0x67,0x09,0x4E,0xFB,0x52,0xA1,0x00,0x00};
#endif

// 开始时间
#if ENGLISH_VERSION
#define C_BEGTIME_LEN 26
uint8_t begtime_char[26] = {0x00,0x53,0x00,0x74,0x00,0x61,0x00,0x72,0x00,0x74,0x00,0x69,0x00,0x6E,0x00,0x67,0x00,0x20,0x00,0x74,0x00,0x69,0x00,0x6D,0x00,0x65};
#else
#define C_BEGTIME_LEN 10
...
void disp_taskname(uint8_t *name){
    // 任务名称
#if ENGLISH_VERSION
    #define C_TASK_NAME_LEN 18
    uint8_t task_name_char[]={0x00,0x54,0x00,0x61,0x00,0x73,0x00,0x6B,0x00,0x20,0x00,0x6E,0x00,0x61,0x00,0x6D,0x00,0x65};
#else
    #define C_TASK_NAME_LEN 10
    uint8_t task_name_char[]={0x4E,0xFB,0x52,0xA1,0x54,0x0D,0x79,0xF0,0x00,0x3A};
#endif
    //------------------------------------------------------------------------------------
    //获取任务名
    memcpy(&g_sys_val.dispname_buff[g_sys_val.disp_num][0],task_name_char,C_TASK_NAME_LEN);
    uint8_t data_base=C_TASK_NAME_LEN;
    //
    uint8_t inc=0;
...
void disp_task_time(){
    uint8_t data_base=0;
    //开始时间
    memcpy(g_sys_val.disinfo2buf[g_sys_val.disp_num],begtime_char,C_BEGTIME_LEN); 
    data_base+=C_BEGTIME_LEN;
    //
...
void user_disptask_refresh(){
    ...
    if (g_sys_val.disp_num==0){
        // 没有任务
        memcpy(g_sys_val.dispname_buff[0],none_task_char,C_NONE_TASK_LEN);
...
void dhcp_disp_en(){
    //“DHCP打开”     
#if ENGLISH_VERSION
    #define  DHCPEN_CHAR    20
    uint8_t dhcp_enchar[]={0x00,0x44,0x00,0x48,0x00,0x43,0x00,0x50,0x00,0x20,0x00,0x69,0x00,0x73,0x00,0x20,0x00,0x4F,0x00,0x4E}; 
#else    
    #define  DHCPEN_CHAR    14
    uint8_t dhcp_enchar[]={0x00,0x44,0x00,0x48,0x00,0x43,0x00,0x50,0x00,0x3A,0x62,0x53,0x5F,0x00}; 
#endif
    memcpy(&disp_buff[DAT_DISP_BASE],dhcp_enchar,DHCPEN_CHAR);
    disp_len = DAT_DISP_BASE+DHCPEN_CHAR;
...
void ip_conflict_disp(uint8_t state){
    if(state){        
#if ENGLISH_VERSION
        #define C_IPCONFLICT_LEN 22
        uint8_t ipconflict_char[]={0x00,0x49,0x00,0x50,0x00,0x20,0x00,0x63,0x00,0x6F,0x00,0x6E,0x00,0x66,0x00,0x6C,0x00,0x69,0x00,0x63,0x00,0x74};
#else    
        #define C_IPCONFLICT_LEN 10
        uint8_t ipconflict_char[]={0x00,0x49,0x00,0x50,0x51,0xB2,0x7A,0x81,0x00,0x21};
#endif
        memcpy(&disp_buff[DAT_DISP_BASE],ipconflict_char,C_IPCONFLICT_LEN);
        disp_len = DAT_DISP_BASE+C_IPCONFLICT_LEN;
...
void dhcp_getin_disp(){
    // "IP获取中"    
#if ENGLISH_VERSION
    #define C_IPQCQ_LEN 24
    uint8_t disp_char[]={0x00,0x41,0x00,0x63,0x00,0x71,0x00,0x75,0x00,0x69,0x00,0x72,0x00,0x69,0x00,0x6E,0x00,0x67,0x00,0x20,0x00,0x49,0x00,0x50}; 
#else
    #define C_IPQCQ_LEN 10
    uint8_t disp_char[]={0x00,0x49,0x00,0x50,0x83,0xB7,0x53,0xD6,0x4E,0x2D}; 
#endif
    memcpy(&disp_buff[DAT_DISP_BASE],disp_char,C_IPQCQ_LEN);
    disp_len = DAT_DISP_BASE+C_IPQCQ_LEN;
    send_buff(DISP_IPCONFILCT_ID);
}
...
#if ENGLISH_VERSION
    // “IP获取成功”
    #define C_DISPSUCCSE_LEN    48
    uint8_t disp_char_succse[]={0x00,0x49,0x00,0x50,0x00,0x20,0x00,0x61,0x00,0x63,0x00,0x71,0x00,0x75,0x00,0x69,0x00,0x73,0x00,0x69,0x00,0x74,0x00,0x69,0x00,0x6F,
                                0x00,0x6E,0x00,0x20,0x00,0x73,0x00,0x75,0x00,0x63,0x00,0x63,0x00,0x65,0x00,0x65,0x00,0x64,0x00,0x65,0x00,0x64}; 
    // “IP获取失败”
    #define C_DISPFAIL_LEN    42
    uint8_t disp_char_fail[]={0x00,0x49,0x00,0x50,0x00,0x20,0x00,0x61,0x00,0x63,0x00,0x71,0x00,0x75,0x00,0x69,0x00,0x73,0x00,0x69,0x00,0x74,0x00,0x69,0x00,0x6F,0x00,
                              0x6E,0x00,0x20,0x00,0x66,0x00,0x61,0x00,0x69,0x00,0x6C,0x00,0x65,0x00,0x64};
#else   
    // “IP获取成功”
    #define C_DISPSUCCSE_LEN    12
    uint8_t disp_char_succse[]={0x00,0x49,0x00,0x50,0x83,0xB7,0x53,0xD6,0x62,0x10,0x52,0x9F}; 
    // “IP获取失败”
    #define C_DISPFAIL_LEN    12
    uint8_t disp_char_fail[]={0x00,0x49,0x00,0x50,0x83,0xB7,0x53,0xD6,0x59,0x31,0x8D,0x25};
#endif
    if(state){
        memcpy(&disp_buff[DAT_DISP_BASE],disp_char_succse,C_DISPSUCCSE_LEN);
        disp_len = DAT_DISP_BASE+C_DISPSUCCSE_LEN;
    }else{
        memcpy(&disp_buff[DAT_DISP_BASE],disp_char_fail,C_DISPFAIL_LEN);
        disp_len = DAT_DISP_BASE+C_DISPFAIL_LEN;
    }
    send_buff(DISP_IPCONFILCT_ID);
}
...
void wifi_open_disp(){
    //wifi 已打开
#if ENGLISH_VERSION
    #define C_WIIFOPEN_LEN 20
    char wifiopen[]={0x00,0x57,0x00,0x69,0x00,0x66,0x00,0x69,0x00,0x20,0x00,0x69,0x00,0x73,0x00,0x20,0x00,0x6F,0x00,0x6E};//14
#else
    #define C_WIIFOPEN_LEN 14
    char wifiopen[]={0x00,0x57,0x00,0x49,0x00,0x46,0x00,0x49,0x5D,0xF2,0x62,0x53,0x5F,0x00};//14
#endif
    memcpy(&disp_buff[DAT_DISP_BASE],wifiopen,C_WIIFOPEN_LEN);
    //itoa_forutf16(host_info.mac[4],&disp_buff[DAT_DISP_BASE+30],16,2);
    disp_len = DAT_DISP_BASE+C_WIIFOPEN_LEN;
    send_buff(DISP_DHCPS_ID);
...
#if ENGLISH_VERSION
    #define C_RESETCHAR_LEN 24
    char resetchar[]={0x00,0x53,0x00,0x79,0x00,0x73,0x00,0x74,0x00,0x65,0x00,0x6D,0x00,0x20,0x00,0x72,0x00,0x65,0x00,0x73,0x00,0x65,0x00,0x74}; // 18 len ,0x00,0x25}; //正在复位主机数据 
#else
    #define C_RESETCHAR_LEN 18
...
#endif
    memcpy(&disp_buff[DAT_DISP_BASE],resetchar,C_RESETCHAR_LEN);
    disp_len = DAT_DISP_BASE+C_RESETCHAR_LEN;
